...
if ! $needs_laravel && ! $needs_symfony && ! $needs_express && ! $needs_solidstart; then
...
if ($needs_express || $needs_solidstart) && ! command -v npm >/dev/null 2>&1; then
    echo "npm is required to prepare Express/SolidStart skeletons." >&2
    exit 1
fi
