...
if ! $needs_laravel && ! $needs_symfony && ! $needs_express && ! $needs_streamlit; then
...
if $needs_streamlit && ! python3 -m pip --version >/dev/null 2>&1; then
    echo "pip is required to prepare the Streamlit skeleton." >&2
    exit 1
fi
