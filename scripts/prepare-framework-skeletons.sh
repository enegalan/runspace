...
gorilla_mux_ready() {
    [[ -f "$GORILLA_MUX_DEST/go.mod" ]] &&
        [[ -f "$GORILLA_MUX_DEST/go.sum" ]] &&
        [[ -f "$GORILLA_MUX_DEST/skeleton.version" ]]
}
...
if force_sync || ! gorilla_mux_ready; then
    needs_gorilla_mux=true
fi

if ! $needs_laravel && ! $needs_symfony && ! $needs_express && ! $needs_gorilla_mux; then
...
if $needs_gorilla_mux && ! command -v go >/dev/null 2>&1; then
    echo "Go is required to prepare the Gorilla Mux skeleton." >&2
    exit 1
fi
...
if $needs_gorilla_mux && [[ ! -f "$GORILLA_MUX_SRC/go.sum" ]]; then
    echo "Generating Gorilla Mux skeleton..."
    rm -rf "$GORILLA_MUX_SRC"
    mkdir -p "$GORILLA_MUX_SRC"
    (
        cd "$GORILLA_MUX_SRC"
        go mod init github.com/runspace/gorilla-mux-sandbox
