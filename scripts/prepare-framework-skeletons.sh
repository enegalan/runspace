...
if ! $needs_laravel && ! $needs_symfony && ! $needs_express && ! $needs_adonisjs; then
...
if ($needs_express || $needs_adonisjs) && ! command -v npm >/dev/null 2>&1; then
    echo "npm is required to prepare the Express and AdonisJS skeletons." >&2
    exit 1
fi
