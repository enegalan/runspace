...
if ! $needs_laravel && ! $needs_symfony && ! $needs_express && ! $needs_meteor; then
...
if ($needs_express || $needs_meteor) && ! command -v npm >/dev/null 2>&1; then
    echo "npm is required to prepare Express/Meteor skeletons." >&2
    exit 1
fi
