...
react_native_ready() {
    [[ -f "$REACT_NATIVE_DEST/package.json" ]] &&
        [[ -f "$REACT_NATIVE_DEST/package-lock.json" ]] &&
        [[ -f "$REACT_NATIVE_DEST/skeleton.version" ]]
}
...
if force_sync || ! react_native_ready; then
    needs_react_native=true
fi

if ! $needs_laravel && ! $needs_symfony && ! $needs_express && ! $needs_react_native; then
...
if $needs_react_native && ! command -v npm >/dev/null 2>&1; then
    echo "npm is required to prepare the React Native skeleton." >&2
    exit 1
fi
...
if $needs_react_native && [[ ! -d "$REACT_NATIVE_SRC/node_modules" ]]; then
    echo "Generating React Native skeleton..."
    rm -rf "$REACT_NATIVE_SRC"
    mkdir -p "$REACT_NATIVE_SRC"
    (
        cd "$REACT_NATIVE_SRC"
        npm init -y --scope=runspace
        npm pkg set name="@runspace/react-native-sandbox"
        npm pkg set description="Internal React Native sandbox for Runspace"
        npm pkg set private=true
        npm install "react-native@${REACT_NATIVE_VERSION}" --save
    )
fi
