...
if ! $needs_laravel && ! $needs_symfony && ! $needs_laminas && ! $needs_express; then
    echo "Framework skeletons already present; skipping generation."
    exit 0
fi

if ($needs_laravel || $needs_symfony || $needs_laminas) && ! command -v composer >/dev/null 2>&1; then
    echo "Composer is required to prepare Laravel/Symfony/Laminas skeletons." >&2
