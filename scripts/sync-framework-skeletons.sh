...
if [[ -f "$QUARKUS_SRC/pom.xml" ]]; then
    mkdir -p "$QUARKUS_DEST"
    rsync -a --delete "${RSYNC_EXCLUDES[@]}" "$QUARKUS_SRC/" "$QUARKUS_DEST/"
    echo "$SKELETON_VERSION" > "$QUARKUS_DEST/skeleton.version"
    synced+=("Quarkus")
fi
