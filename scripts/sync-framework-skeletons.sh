...
if [[ -f "$MICRONAUT_SRC/pom.xml" ]]; then
    mkdir -p "$MICRONAUT_DEST"
    rsync -a --delete "${RSYNC_EXCLUDES[@]}" "$MICRONAUT_SRC/" "$MICRONAUT_DEST/"
    echo "$SKELETON_VERSION" > "$MICRONAUT_DEST/skeleton.version"
    synced+=("Micronaut")
fi
