...
if [[ -f "$SPRING_BOOT_SRC/pom.xml" ]]; then
    mkdir -p "$SPRING_BOOT_DEST"
    rsync -a --delete "${RSYNC_EXCLUDES[@]}" "$SPRING_BOOT_SRC/" "$SPRING_BOOT_DEST/"
    echo "$SKELETON_VERSION" > "$SPRING_BOOT_DEST/skeleton.version"
    synced+=("Spring Boot")
fi
