...
if [[ -d "$NEXTJS_SRC/node_modules" ]]; then
    mkdir -p "$NEXTJS_DEST"
    rsync -a --delete --exclude node_modules/ --exclude .git/ "$NEXTJS_SRC/" "$NEXTJS_DEST/"
    echo "$SKELETON_VERSION" > "$NEXTJS_DEST/skeleton.version"
    synced+=("Next.js")
fi
