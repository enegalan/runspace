...
if [[ -f "$DROPWIZARD_SRC/pom.xml" ]]; then
    mkdir -p "$DROPWIZARD_DEST"
    rsync -a --delete "${RSYNC_EXCLUDES[@]}" "$DROPWIZARD_SRC/" "$DROPWIZARD_DEST/"
    echo "$SKELETON_VERSION" > "$DROPWIZARD_DEST/skeleton.version"
    synced+=("Dropwizard")
fi
