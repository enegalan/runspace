...
if [[ -d "$ADONISJS_SRC/node_modules" ]]; then
    mkdir -p "$ADONISJS_DEST"
    rsync -a --delete --exclude node_modules/ --exclude .git/ "$ADONISJS_SRC/" "$ADONISJS_DEST/"
    echo "$SKELETON_VERSION" > "$ADONISJS_DEST/skeleton.version"
    synced+=("AdonisJS")
fi
