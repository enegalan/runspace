...
if [[ -d "$REACT_NATIVE_SRC/node_modules" ]]; then
    mkdir -p "$REACT_NATIVE_DEST"
    rsync -a --delete --exclude node_modules/ --exclude .git/ "$REACT_NATIVE_SRC/" "$REACT_NATIVE_DEST/"
    echo "$SKELETON_VERSION" > "$REACT_NATIVE_DEST/skeleton.version"
    synced+=("React Native")
fi
