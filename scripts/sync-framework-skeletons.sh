...
if [[ -f "$QUART_SRC/requirements.txt" ]]; then
    mkdir -p "$QUART_DEST"
    rsync -a --delete --exclude vendor/ --exclude .venv/ --exclude .git/ "$QUART_SRC/" "$QUART_DEST/"
    echo "$SKELETON_VERSION" > "$QUART_DEST/skeleton.version"
    synced+=("Quart")
fi
