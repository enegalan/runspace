...
if [[ -f "$PYRAMID_SRC/requirements.txt" ]]; then
    mkdir -p "$PYRAMID_DEST"
    rsync -a --delete --exclude vendor/ --exclude .venv/ --exclude .git/ "$PYRAMID_SRC/" "$PYRAMID_DEST/"
    echo "$SKELETON_VERSION" > "$PYRAMID_DEST/skeleton.version"
    synced+=("Pyramid")
fi
