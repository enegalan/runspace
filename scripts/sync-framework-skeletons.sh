...
if [[ -f "$STARLETTE_SRC/requirements.txt" ]]; then
    mkdir -p "$STARLETTE_DEST"
    rsync -a --delete --exclude vendor/ --exclude .venv/ --exclude .git/ "$STARLETTE_SRC/" "$STARLETTE_DEST/"
    echo "$SKELETON_VERSION" > "$STARLETTE_DEST/skeleton.version"
    synced+=("Starlette")
fi
