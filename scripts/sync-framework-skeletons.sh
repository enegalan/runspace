...
if [[ -d "$TORNADO_SRC/site-packages" ]]; then
    mkdir -p "$TORNADO_DEST"
    rsync -a --delete --exclude site-packages/ --exclude .git/ "$TORNADO_SRC/" "$TORNADO_DEST/"
    echo "$SKELETON_VERSION" > "$TORNADO_DEST/skeleton.version"
    synced+=("Tornado")
fi
