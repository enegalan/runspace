...
if [[ -f "$SANIC_SRC/requirements.txt" ]]; then
    mkdir -p "$SANIC_DEST"
    rsync -a --delete --exclude vendor/ --exclude .venv/ --exclude .git/ "$SANIC_SRC/" "$SANIC_DEST/"
    echo "$SKELETON_VERSION" > "$SANIC_DEST/skeleton.version"
    synced+=("Sanic")
fi
