...
if [[ -d "$SVELTEKIT_SRC/node_modules" ]]; then
    mkdir -p "$SVELTEKIT_DEST"
    rsync -a --delete --exclude node_modules/ --exclude .git/ "$SVELTEKIT_SRC/" "$SVELTEKIT_DEST/"
    echo "$SKELETON_VERSION" > "$SVELTEKIT_DEST/skeleton.version"
    synced+=("SvelteKit")
fi
