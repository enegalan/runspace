...
if [[ -f "$DASH_SRC/requirements.txt" ]]; then
    mkdir -p "$DASH_DEST"
    rsync -a --delete --exclude vendor/ --exclude .venv/ --exclude .git/ "$DASH_SRC/" "$DASH_DEST/"
    echo "$SKELETON_VERSION" > "$DASH_DEST/skeleton.version"
    synced+=("Dash")
fi
